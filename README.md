# Hi, I'm Max 👋

📍 Miami ↔ Barranquilla | 🛠 Product Builder | 🚀 15+ years shipping software

![Rails](https://img.shields.io/badge/-Rails-CC0000?style=flat&logo=ruby-on-rails&logoColor=white)
![Swift](https://img.shields.io/badge/-Swift-FA7343?style=flat&logo=swift&logoColor=white)
![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)
![AI](https://img.shields.io/badge/-AI-412991?style=flat&logo=openai&logoColor=white)

> Building AI-native products and tools. Founder-mode product thinker — strategy, UX, architecture. Recently exited a services company, now running a portfolio of founder-led products.

## Current Projects

- 🦞 **[ClawDeck](https://clawdeck.io)** — Open source AI-native task management, built for [OpenClaw](https://github.com/openclaw/openclaw) agents
- 🎨 **[mx.works](https://mx.works)** — AI-native product & design studio
- 🔗 **[tini.bio](https://tini.bio)** — Your link in bio that's actually a website
- 🧠 **[tidi.so](https://tidi.so)** — A mind organizer: organize and search images, notes, anything
- 🙏 **[Gratu](https://gratu.app)** — Minimal gratitude journaling

## What I Do

I define, design, and build products — especially AI-driven ones. Not an operator or CEO type; I'm the builder and product partner in the room.

**Strengths:** Product strategy • UX/Design • Software architecture • AI systems • Turning vague ideas into shipped products

## Connect

- 𝕏 [@andresmax](https://x.com/andresmax)
- 🌐 [andresmax.com](https://andresmax.com)

---

*Family > Work. Always.*
